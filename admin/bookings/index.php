...
<div class="card card-outline rounded-0 card-navy">
	<div class="card-header ">
		<div class="card-tools d-flex justify-content-end">
			<a href="<?= base_url ?>admin?page=bookings/manage_booking" id="create_new" class="btn btn-flat btn-primary bg-gradient-teal border-0 rounded00"><span class="fas fa-plus"></span> Crear Reserva</a>
		</div>
	</div>
	<div class="card-body">
						...
						?>
							<tr>
								...
								<td class="align-items-center text-end">
									<p class="text-center truncate-1" style="max-width:250px"><?= implode(", ", $services_arr) ?></p>
								</td>
								<td class="align-items-center justify-content-center text-center">
									<?php if ($row['status'] == 1) : ?>
										<span class="badge bg-primary px-3 rounded-pill">Confirmado</span>
									<?php elseif ($row['status'] == 2) : ?>
										<span class="badge bg-danger px-3 rounded-pill">Cancelado</span>
									<?php else : ?>
										<span class="badge bg-secondary px-3 rounded-pill">Pendiente</span>
									<?php endif; ?>
								</td>
								<td class="align-items-center" align="center">
									<div class="dropdown">
										...
										<div class="dropdown-menu" role="menu">
											<a class="dropdown-item" href="./?page=bookings/view_booking&id=<?php echo $row['id'] ?>"><span class="bi bi-card-text text-dark"></span> Ver</a>
											<div class="dropdown-divider"></div>
											<a class="dropdown-item" href="./?page=bookings/manage_booking&id=<?php echo $row['id'] ?>"><span class="bi bi-pencil-square text-primary"></span> Editar</a>
										</div>
									</div>
								</td>
							</tr>
...
<script>
	$(document).ready(function() {
		$('.delete_data').click(function() {
			_conf("¿Deseas eliminar esta reserva de forma permanente?", "delete_booking", [$(this).attr('data-id')])
		})
